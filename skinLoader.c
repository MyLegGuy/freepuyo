...
struct puyoSkin lowLoadSkinFile(crossTexture _passedImage, int _numColors, int _xStart, int _yStart, int _xSeparation, int _ySeparation, int _singlePuyoW, int _singlePuyoH){
...
struct puyoSkin loadSkinFileChronicle(const char* _passedFilename){
	return lowLoadSkinFile(loadImage(_passedFilename),5,0,0,18,18,16,16);
}
